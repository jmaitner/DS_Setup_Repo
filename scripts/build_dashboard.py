  ...
  if(view==='catalog'){
    ...
    tbody.innerHTML=rows.map(d=>`<tr data-i="${DATA.indexOf(d)}">
      ...
      ${Object.keys(CHANNELS).map(k=>{const c=d.channels[k];
        const tip=`${CHANNELS[k]}: ${c.state}`+(c.id?` · ${c.id}`:'')+(c.case?` · case ${c.case}`:'')+(c.issue?` · ${c.issue}`:'');
        return `<td><span class="chip" style="background:${cssVar(c.state)}" title="${esc(tip)}"></span></td>`;}).join('')}
    </tr>`).join('');
  }
}
